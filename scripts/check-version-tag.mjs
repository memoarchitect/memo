...
if (ref.startsWith('refs/tags/')) {
  ...
  if (pushedTag !== expectedTag) {
    fail(`Tag ${pushedTag} does not match package version ${pkg.version}; expected ${expectedTag}.`);
  }
...
let previousVersion;
try {
  previousVersion = JSON.parse(runGit('show', `${before}:package.json`)).version;
} catch {
  console.log('The previous commit has no readable package version; comparison skipped.');
  process.exit(0);
}
...
if (!tags.includes(expectedTag)) {
  fail(`package.json changed from ${previousVersion} to ${pkg.version}, but HEAD is not tagged ${expectedTag}. Push the main commit and ${expectedTag} together.`);
}
